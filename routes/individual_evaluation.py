...
def individual_evaluation():
    ...
    try:
        ...
        return render_template('individual_evaluation.html', teams=assigned_teams, tier=session.get('tier', 0))
    except Exception as e:
        print(f"Error loading individual evaluation page: {e}")
        return jsonify({'error': 'An internal error occurred'}), 500

@individual_evaluation_bp.route('/get_assigned_teams', methods=['GET'])
def get_assigned_teams():
    ...
    try:
        ...
        print(f"Error in get_assigned_teams: {e}")
        return jsonify({'error': 'An internal error occurred'}), 500

@individual_evaluation_bp.route('/get_team_members/<int:team_id>', methods=['GET'])
