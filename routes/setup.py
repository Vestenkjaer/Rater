...
def create_user():
    try:
        ...
        # Check if user already exists
        existing_user = User.query.filter_by(email=email, client_id=client_id).first()
        if existing_user:
            current_app.logger.error('User with this email already exists')
            return jsonify({'error': 'User with this email already exists'}), 400
        ...
        # Store user with auth0_id in local database
        new_user = User(username=username, email=email, client_id=client_id, auth0_id=auth0_id)
        ...
        for team_id in team_ids:
            team = Team.query.get(team_id)
            if team and team.client_id == client_id:
                user_team = user_teams.insert().values(user_id=new_user.id, team_id=team.id)
                db.session.execute(user_team)
                team_names.append(team.name)
                db.session.commit()
